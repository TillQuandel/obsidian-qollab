...
| **Three or more devices — loss of text nobody touched.** Confirmed on the current version. | Baseline lines missing at the end of a run, out of 3,200 per run (40 seeds × 10 notes × 8 lines): **2.0 with 3 devices** (range 1–3, 6 runs), **8.0 with 4** (range 3–11, 5 runs). Zero in 1 of 14 runs; above zero in the other 13. The fix that removed the two-device failure (296 of 720 runs → 0) does **not** cover this one. With **two** devices the loss is 0 in every run. Earlier figures for this row (4.6 and 19.7) were measured with a harness that skipped a call the plugin makes after every write-back, and are superseded. | Not necessarily |
