...
| **Four or more devices — loss of text nobody touched** | Baseline lines missing at the end of a run, out of 3,200 per run (40 seeds × 10 notes × 8 lines): **1.2 with 4 devices** (range 1–2), in every one of 5 runs. With **two or three** devices: **0 in every run** — and with two devices the run is now completely clean (40/40 seeds, no duplication at all). Earlier figures for this row (4.6 / 19.7, later 2.0 / 8.0, and the claim that three devices were affected) came from a harness that processed a synced-in `.md` as if you had typed it, which the plugin does not do. They are **superseded**. | Not necessarily |
