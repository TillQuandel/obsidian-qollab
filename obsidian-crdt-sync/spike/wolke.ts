...
export class Wolke {
  ...
  // 'kopie'         — der Datei-Sync loest konkurrierende `.md`-Schreibvorgaenge
  //                   wie OneDrive: Wolken-Fassung gewinnt lokal, die eigene wird
  //                   zur Kopie.
  // 'ohne'          — der `.md`-Kanal wird stillgelegt, solange eine Seite
  //                   unsynchrone lokale Aenderungen hat. Damit traegt allein der
  //                   CRDT-Kanal. (Derselbe Kunstgriff wie `noMdConflict` in den
  //                   Vorarbeiten — kuenstlich, aber notwendig: mit
  //                   Konfliktkopien ist die KONTROLLE genauso rot wie der
  //                   Testfall.)
  // 'ueberschreiben'— der Rueckfall OHNE Netz: die Wolken-Fassung ersetzt die
  //                   lokale, und es entsteht KEINE Kopie. Das ist keine
  //                   Erfindung, sondern die Lage, die die deutsche
  //                   README-Fassung neben dem OneDrive-Fall ausdruecklich nennt
  //                   („eine Fassung wird aus dem Versionsverlauf
  //                   wiederhergestellt, ein Lesevorgang liefert die Datei
  //                   verkuerzt") — und die Lage jedes Nutzers, der die
  //                   Konfliktkopie-Sicherung seines Dienstes abgeschaltet hat.
  konfliktModus: 'kopie' | 'ohne' | 'ueberschreiben' = 'kopie';
  ...
  ladeMdHoch(g: Geraet, notePath: string): void {
    ...
    if (eintrag && eintrag.v > s.v) {
      ...
      // Ohne Netz gibt es keine Kopie — der lokale Stand wird beim naechsten
      // Herunterladen ersatzlos ueberschrieben.
      if (this.konfliktModus !== 'ueberschreiben') this.kopien.get(g.id)!.push(lokal);
      return;
    }
    const v = ++this.version;
  ...
  ladeMdHerunter(g: Geraet, notePath: string): boolean {
    ...
    if (lokal !== undefined && s.text !== undefined && lokal !== s.text) {
      ...
      // Sonst: die Wolken-Fassung ersetzt die lokale. Ob unsere davor als Kopie
      // gerettet wird, ist genau der Unterschied zwischen 'kopie' (OneDrive) und
      // 'ueberschreiben' (kein Netz).
      if (this.konfliktModus !== 'ueberschreiben') this.kopien.get(g.id)!.push(lokal);
    }
