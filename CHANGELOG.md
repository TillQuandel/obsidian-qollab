...
- `.md`-Overwrite durch Datei-Sync duplizierte Remote-Edits, wenn die Sidecar noch nicht gemergt war (empirisch im Zwei-Vault-Realtest belegt): Lieferte robocopy die bereits gemergte `.md` des Peers *zugleich* mit dessen ungemergter `.yjs`, diffte `applyLocalContent` den gemergten `.md`-Text gegen den eigenen Doc (der die Fremd-Ops noch nicht hatte) und erzeugte die Fremd-Einfügung als neue lokale Op unter eigener Client-ID. Spielte der `SidecarWatcher` später die Fremd-Sidecar ein, dedupliziert Yjs nach Item-ID (nicht Inhalt) → der Fremd-Edit stand dauerhaft doppelt im CRDT. `applyLocalContent` mergt jetzt bei bereits geladenem Doc erst ausstehende kompatible Fremd-Sidecars ein (reine `mergeCompatible`-Semantik, kein Tie-Break), bevor der lokale Diff berechnet wird — der Diff der sync-gelieferten `.md` gegen den aktualisierten Doc ist dann leer, echte Simultan-Edits werden korrekt „erst remote mergen, dann lokal draufdiffen" behandelt.
